###11/15/21 increased zerocharge interpolation resolution to match IFT resolution
###11/15/21 redefined the charge carrier mass correction to reflect exact proton mass to 3 decimal places
...
def zerocharge (IFT,x,cs,proton):
    """
    ...
    try:
        ...
        if cs == 0:
            csmin = 1
            csmax = 1
    xmin = round(x[np.min(indices)]*csmin,3)
    xmax = round(x[np.max(indices)]*csmax,3)
    density = (x[2]-x[1])*csmin
    length = int(round(1.2*(xmax-xmin)/density))
    xzero = np.round(np.linspace(xmin,xmax,length),3)
    ...
    try:
        for i in range(len(cs)):
            if cs[i]==0:
                ...
                xtemp = (x-proton)*cs[i] ### normalizes for charge and corrects mass for charge carriers (protons)
        ...
        if cs==0:
            ...
            xtemp = (x-proton)*cs ### normalizes for charge and corrects mass for charge carriers (protons)
        ytemp = np.interp(xzero,xtemp,IFT)
        cszero.append(ytemp)
        zerofull += ytemp
...
def cszero_score(cszero):
    "Charge assignment score for charge adjuster"
    score = np.round(np.log(np.sum(np.absolute(np.prod(cszero,axis=0)))))
    if str(score) == '-inf':
        score = 0
    try:
        score = int(score)
